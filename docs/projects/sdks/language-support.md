...
| **TypeScript** | `avalon-sdks` repo's `languages/typescript/` (`@avalon-initiative/protocol-sdk`) | **Fully supported** | Real and shipped, browser-facing — drives a real WebAuthn ceremony (unlike C#). A self-contained package/`apps/hub`. What `apps/hub` runs on, so it's exercised by a real production frontend, not only its own test suite. See [`typescript/README.md`](typescript/README.md). |
