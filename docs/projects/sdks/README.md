...
| C# (`avalon-sdks`' `csharp/AvalonSdk`, not `bindings/csharp` in this repo — moved by #775, netstandard2.1, Unity-targeted) | [`csharp/`](csharp/README.md) | Real, building, tested — 126+ passing tests including opt-in live ones. The priority developer-facing surface, since it's what most integrating game studios will actually use. |
| TypeScript (`avalon-sdks`' `typescript/`, not `bindings/ts` in this repo — moved by #775, published as `@avalon-initiative/protocol-sdk` on GitHub Packages) | [`typescript/`](typescript/README.md) | Real and shipped, browser-facing — drives a real WebAuthn ceremony. Also what `apps/hub` runs on (as a real published dependency now, not a local path), so it's exercised by a real production frontend, not only its own test suite. |
