...
|---|---|---|
| Friendship (identity A ↔ identity B) | **Yes** | durable network relationship, owned by the network, not either identity's current integrator |
...
**Conversations.** The same never-reveal rule extends to direct/small-group
...
- `crates/server/src/friends.rs` — session-authenticated endpoints:
  ...
  `GET /friends/requests`. There is no integrator-credential auth path for
  this domain, so "an integrator cannot act on an identity's behalf" holds by
  ...
  through the outbox (`crates/server/src/outbox.rs`) in the same
  ...
  Ed25519 signature; see [protocol-events.md](./protocol-events.md).
...
- A short handle resolves an identity without pasting a raw id: `GET
  /friends/handle/:handle` resolves it to an identity id — exact match only,
  session-authenticated like every other route in this module.
  `display_name` itself is the handle — globally unique, case-insensitive
  (`profiles_display_name_lower_idx`), no discriminator suffix. Uniqueness
  is enforced by that index at the actual write
  ...
  never an auto-suggested variant. This follows Discord's current scheme
  (a globally-unique handle) rather than the deprecated `name#1234` one it
  ...
  is out of scope here — that's identity discovery (below).
- **Identity discovery is two-tier**: private-by-default, always-on
  surfacing of identities via mutual friends ("friends of friends") and
  shared guild membership — never a name search, just relationships that
  already exist — plus a first-class, easily reversible per-identity toggle
  ("make me publicly searchable") that opts an identity into open
  name/handle search — off by default, matches only opted-in identities when
  on, and flips off just as easily as it flips on.
  - `crates/server/src/discovery.rs` — `GET /people/discover`, the always-on
    half. Session-authenticated with no query parameter of any kind — the
    only input is the caller's own session, never a search term. Candidates
    come from two sources, unioned and deduplicated: friends-of-friends
    (`friendships` rows touching one of the caller's own friends) and mutual
    guild membership (`guild_members` rows sharing a `guild_id` with the
    caller). Both sources are filtered through the caller themselves,
    `friends::friend_partners`, and `blocks::block_partners` — the same
    "compute related identities" helpers presence uses, reused rather than
    reimplemented, so a blocked relationship is excluded from discovery
    with the identical guarantee it already gets from presence and friend
    requests. A direct query today, not yet the real indexer read model.
    Read-only — acting on a suggestion still goes through `POST
    /friends/requests` unchanged; this endpoint never creates or modifies a
    friendship.
  - `crates/server/src/discovery.rs` — `GET /identities/search?q=&limit=`,
    the opt-in half. Session-authenticated, matches only identities with
    `discovery_preferences.discoverable = true` (an identity-controlled
    preference, off by default for every identity, no exceptions — a row
    created lazily on first toggle, absence means the default). Toggled via
    `PATCH /me`'s `discoverable` field
    (`crates/server/src/handlers.rs::update_profile`). Fuzzy, case-insensitive
    `ILIKE` substring match against `display_name` (the handle itself, no
    separate discriminator suffix to also match). A non-opted-in identity
    never appears, full stop, even to a caller who already knows its exact
    ...
    either direction via `blocks::block_partners`, same reuse
    `discover_people` above establishes. Turning the toggle off removes an
    identity from every subsequent search call immediately — the preference
    read is a plain `WHERE discoverable = true` against `&state.pool` on
    every call, not a cached/snapshotted value, so there is no grace
    period. `GET /me` also returns the caller's own current `discoverable`
    value, so the Hub's "you are currently publicly searchable" indicator
    can never drift out of sync with the stored preference.
- Reads are restricted to the caller's own session for now — the full
  capability/visibility composition described in "What an integrator sees"
  above is not built yet, so there is no integrator-facing read path at all.
- The Rust SDK (`avalon-sdks` repository)'s `Session::friends()`,
  `presence()`/`presence_of()`, `update_presence()`, and
  `subscribe_presence()`, each gated behind `Session::require(capability)`.
- `crates/server/src/blocks.rs` + `crates/server/db/migrations/0006_blocks/`
  — `blocks (blocker, blocked, created_at)`, `POST /blocks`,
  ...
  `conversations::send_message` via the group-aware `blocks::has_block_among`
  — see "Blocking and harassment" above. Reachable from the Hub:
  `UserProfile.vue`'s actions row (Add/Remove friend, Block/Unblock,
  mirroring whichever relationship actually exists between the caller and
  the viewed identity) and a "Blocked users" card on `Profile.vue` (list +
  unblock, plus block by id/handle directly).
