...
| [`sdks/`](sdks/README.md) | Every official SDK: the Rust reference SDK (moved to the `avalon-sdks` repo's `rust/` by issue #775, epic #771 — no longer `crates/sdk` here), the C# SDK (`bindings/csharp/AvalonSdk`, Unity-targeted), and the TypeScript SDK (`bindings/ts`, browser-facing), plus one shared design reference. One project, not one per language — see below for why. | Rust SDK real and live-tested, with independent wire types/signing logic (#774) verified against a shared conformance suite. C# SDK real, building, tested (52+ passing tests) as of 2026-09-20. TypeScript SDK real and shipped (#701), and what `apps/hub` is actively migrating onto (#712). |
