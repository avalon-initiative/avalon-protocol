...
| [`sdks/`](sdks/README.md) | Every official SDK: the Rust reference SDK (`crates/sdk`) and the C# SDK (`bindings/csharp/AvalonSdk`, Unity-targeted), plus one shared design reference. One project, not one per language — see below for why. | Rust SDK real and live-tested; achievement issuance still `NotImplemented`. C# SDK real, building, tested (52+ passing tests) as of 2026-09-20. |
