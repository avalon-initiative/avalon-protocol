...
| [`sdks/`](sdks/README.md) | Every official SDK: the Rust reference SDK (`crates/sdk`), the C# SDK (`bindings/csharp/AvalonSdk`, Unity-targeted), and the TypeScript SDK (`bindings/ts`, browser-facing), plus one shared design reference. One project, not one per language — see below for why. | Rust SDK real and live-tested; achievement issuance still `NotImplemented`. C# SDK real, building, tested (52+ passing tests) as of 2026-09-20. TypeScript SDK real and shipped (#701), and what `apps/hub` is actively migrating onto (#712). |
