...
- `apps/hub/` — Vue 3 + Vite + TypeScript, routed with `vue-router`. Identity
  ...
  pre-authenticated pages — see [identity](../../backend-server/architecture/identity.md)'s "Today in the
...
- `Friends.vue` (#18) keeps each friend's presence live via
  ...
  server-side (see
  [social-graph](../../backend-server/architecture/social-graph.md)/[presence](../../backend-server/architecture/presence.md)), the same
...
- `Messages.vue` (#105) — direct/small-group conversations
  ([communication.md](../../backend-server/architecture/communication.md)): a conversation-list sidebar next
...
- `Activity.vue` (#121) — "what does the network know about me": the
  ...
  [settlement](../../backend-server/architecture/settlement.md)'s "Today in the repo" for why). Not a
...
- `Guilds.vue` / `Guild.vue` / `GuildChannel.vue` (#24) — `/guilds` (my
  ...
  [guilds](../../backend-server/architecture/guilds.md)'s "Today in the repo" for the two real gaps this
...
- `RecoverIdentity.vue` (`/recover-identity`, #201) — the guardian-based
  ...
  [identity](../../backend-server/architecture/identity.md)'s "Today in the repo" for the server-side
  mechanics.
