...
Avalon is an open network for games that gives developers shared player
...
presence, player profiles, and moderation — and then discovers it also needs
...
chat system," a game consumes "this is the player's Avalon guild, give me its
...
## 2. Players stay connected outside the game
...
particular game is running. A player can be playing Game A, browsing the Hub,
...
of the *only* place. A player in your game and a player sitting in Discord
...
The community follows the player instead of the player following the app.
...
player doesn't have to leave the game to stay connected to the people they
play with.

## 5. Cross-game communities become possible

A guild's membership doesn't have to be defined by a single game's player
list. A game can, in principle, tell a player: "43 members of your guild are
...
## 6. Your game can bring players with it
...
signal than "here's another game in a storefront." Players aren't discovering
...
## 8. Player identity without forcing characters to be universal

Avalon does not require every game to share a character model. A player's
...
provides the identity that connects the player behind those characters, not
the characters themselves.

## 9. Portable history

Players accumulate years of history across the games they play. Avalon lets
...
because players expect them. The intent is for the SDK to expose these as
...
let guild = avalon.player().guild(guild_id).await?;
...
Games shut down, go into maintenance, get sequels, or just lose players. If a
...
A player who stops playing for three months is much more likely to come back
...
infrastructure developers actively build around. Every new game adds players,
...
question flips from "why would I add this" to "why would I make players
...
lock-in — it's to make participation useful enough that players start asking
for it.
...
to try this game" and "I'm socially invested in this game" — a new player
...
effectively its own island. Avalon is the layer that lets players move
...
players can bring their Avalon identity, friends, guilds, communities, and
...
games integrate because their players expect their identity, friends, guilds,
history, and communication to follow them. At that point Avalon isn't just an
SDK — it's part of the infrastructure players expect a modern multiplayer
