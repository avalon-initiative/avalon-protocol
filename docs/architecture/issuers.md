...
| category | `game` · `app` · `service` (#282, decision #275) — additive, defaults to `game`, doesn't rename `integrator id`/issuer vocabulary above |
...
| issuer key | the integrator | attestations the integrator issues | #80 / #84 |
...
- `crates/protocol/src/integrations.rs` — `Integrator { id, slug, name, developer,
  ...
  `IntegratorRegistration { integrator, requested_capabilities, initial_key }`,
  `IssuerKeyInfo { key_id, algorithm, public_key }` (the registration-time
  wire shape), `IntegratorCredential { integrator_id, key_id }`. **#84 (implementing
...
- `crates/server/src/integrations.rs` (#26) — `POST /integrations` registers an integrator: slug
  ...
  the request body; omitted means `game`. Projection tables: `integrators`
  (migration `0040_game_category` added the `category` column, `NOT NULL
  DEFAULT 'game'`), `integrator_requested_capabilities`, and `issuer_keys`
  ...
  `game.registered`'s `issuer`/`subject` name the integrator itself but the event
  is network-attributed, not integrator-signed — see the module doc comment on
  why (the registrant's key isn't proven to control anything yet at that
  point). `POST /integrations/{slug}/challenge` + `authenticate_integrator`/`GET
  /integrations/whoami` implement the milestone-1 challenge-response stand-in
  ...
  (`integrator_challenges`, same ephemeral-ceremony shape as
  `webauthn_ceremonies`), signed by the integrator's registered key, verified via
  `auth::verify_event_signature` — `authenticate_integrator` now also filters out
...
- **Key-set management (#84)**: `POST /integrations/{slug}/keys` (add) and `POST
...
- **`GET /integrations/{slug}/keys` (#90)**: the read side of the above — public,
  ...
  Hub integrator-profile page needed a way to actually show the "key history and
...
- **Still deferred (#84's own explicit scope note)**: the network-level
  authorization model for transitioning `IntegratorStatus` into `Suspended`/
...
- `crates/cli` — `avalon register-integrator --slug <slug> --name <name>
  ...
  (`POST /integrations/{slug}/challenge` → `GET /integrations/whoami`, sending the
  ...
  command — identical parsing and behavior, `register-integrator` still works too.
