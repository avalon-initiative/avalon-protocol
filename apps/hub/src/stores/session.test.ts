...
describe('session store', () => {
  ...
  describe('reconnect credentials (issue #525)', () => {
    it('login persists identityId and signingKeyId alongside the token', () => {
      const session = useSessionStore()
      session.login('a-token', 'identity-1', 'key-1')
    ...
    it('login defaults identityId/signingKeyId to null when omitted', () => {
      const session = useSessionStore()
      session.login('a-token')
    ...
    it('login with a null signingKeyId (no local signing key yet) stores none', () => {
      const session = useSessionStore()
      session.login('a-token', 'identity-1', null)
      expect(session.signingKeyId).toBeNull()
      expect(localStorage.getItem('avalon:session:signingKeyId')).toBeNull()
    })

    it('setSigningKeyId updates just that field, independent of token/identityId', () => {
      const session = useSessionStore()
      session.login('a-token', 'identity-1', null)
      session.setSigningKeyId('key-later')
    ...
    it('logout clears identityId and signingKeyId too', () => {
      const session = useSessionStore()
      session.login('a-token', 'identity-1', 'key-1')
      session.logout()
      expect(session.identityId).toBeNull()
    ...
    it('picks up identityId/signingKeyId already in localStorage on creation', () => {
      localStorage.setItem('avalon:session:token', 'existing-token')
