...
help:
	...
	@echo "  make stack-up      Docker Compose bring-up: postgres + avalon-server, migrated and started, no Rust/Node toolchain needed (issue #289)"
	@echo "  make stack-down    stop what 'make stack-up' started"
...
stack-up:
	...
	docker compose --profile stack up -d --build postgres
	docker compose --profile stack run --rm migrate
	docker compose --profile stack up -d --build avalon-server
	@echo "avalon-server should now be reachable at http://127.0.0.1:8080 — 'make stack-logs' to follow it, 'make stack-down' to stop."

stack-down:
	docker compose --profile stack down

stack-logs:
	docker compose --profile stack logs -f avalon-server
