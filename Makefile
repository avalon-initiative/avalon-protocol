...
# dependency), so unlike the Rust SDK's build.rs this can't regenerate
# automatically at compile time; it's checked in and regenerated by hand,
# same convention as openapi/openapi-check above.
ts-sdk-types:
	cd bindings/ts && npm run generate

ts-sdk-types-check:
	cd bindings/ts && npm run generate:check

# Issue #725: bindings/csharp/AvalonSdk/Generated.cs (C# wire-shape types from
# docs/generated/openapi.json) is a generated artifact too — like bindings/ts,
# AvalonSdk has no build step Unity consumers go through (they import its .cs
# source directly), so this can't regenerate automatically at compile time
# either; it's checked in and regenerated by hand, same convention as
# openapi/openapi-check and ts-sdk-types/ts-sdk-types-check above.
csharp-sdk-types:
	cd bindings/csharp/codegen && dotnet run

csharp-sdk-types-check:
	cd bindings/csharp/codegen && dotnet run
